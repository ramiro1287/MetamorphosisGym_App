...
export default function AddTemplateExerciseModal({ templateId, onClose, reload, setSelectedExercise }) {
  ...
  const handleAddExercise = async (exerciseId) => {
    onClose();
    const confirm = await showConfirmModalAlert("¿Agregar este ejercicio a la plantilla?");
    if (!confirm) return;

    try {
      const response = await fetchWithAuth(
        `/admin/training-plans/templates/exercise-detail/add/`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ plan_id: templateId, exercise_id: exerciseId }),
        }
      );
