...
export default function EditTemplateExerciseModal({ exercise, onClose, reload }) {
  ...
  const validate = () => {
    ...
    if (formData.sets === "") {
      if (formData.reps !== "") {
        newErrors.reps = "No puede haber repeticiones si no hay series";
      }
    } else if (!isPositiveInteger(formData.sets)) {
      newErrors.sets = "Ingrese un número entero mayor a 0";
    }
  ...
  const handleSave = async () => {
    if (!validate()) return;
    onClose();
    const confirm = await showConfirmModalAlert("¿Desea guardar los cambios del ejercicio de la plantilla?");
    if (!confirm) return;

    try {
      const response = await fetchWithAuth(
        `/admin/training-plans/templates/exercise-detail/${exercise.id}/`,
        {
          method: "PUT",
    ...
    <Modal visible transparent animationType="slide" onRequestClose={onClose}>
      <View style={common.modalContainer}>
        <View style={styles.cardModalContainer}>
          <Text style={[styles.cardTitle, { fontSize: 18 }]}>Editar ejercicio de plantilla</Text>
